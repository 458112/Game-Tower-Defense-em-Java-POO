...
public class GerenciadorTorres {
	...
	public void removerTorre(Torre torreExibida) {
		for (int i = 0; i < torres.size(); i++)
			if (torres.get(i).getId() == torreExibida.getId())
				torres.remove(i);
	}
	
	public void atualizarTorre(Torre torreExibida) {
		for (Torre t : torres)
			if (t.getId() == torreExibida.getId())
				t.atualizarTorre();
	}
